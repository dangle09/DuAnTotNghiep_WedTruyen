...
namespace TeenNovel_Wed.Controllers
{
    public class DocGiaController : Controller
    {
        ...
        public async Task<IActionResult> TaoDonNap(int id)
        {
            ...
            var nguoidung = await _context.DocGias
                .Include(x => x.MatkNavigation)
                .FirstOrDefaultAsync(x => x.MaDocGia == int.Parse(User.FindFirst("MaDocGia")!.Value));
            ...
            if (goiNap == null)
            {
                ...
                {
                    success = false,
                    message = "Không tìm thấy gói nạp."
                });
            }
            ...
            string noiDung = $"{nguoidung?.Ten} nạp {goiNap?.SoTien} lúc TN{DateTime.Now:yyyyMMddHHmmss}{Random.Shared.Next(100, 999)}";
            ...
            //    $"?amount={goiNap.SoTien}" +
            //    $"&addInfo={Uri.EscapeDataString(noiDung)}" +
            //    $"&accountName={Uri.EscapeDataString(accountName)}";

            Console.WriteLine("bankBin = " + (bankBin ?? "NULL"));
            Console.WriteLine("bankName = " + (bankName ?? "NULL"));
            Console.WriteLine("accountNumber = " + (accountNumber ?? "NULL"));
            Console.WriteLine("accountName = " + (accountName ?? "NULL"));
            Console.WriteLine("noiDung = " + (noiDung ?? "NULL"));
            Console.WriteLine($"MaGoiNap = {goiNap.MaGoiNap}");
            Console.WriteLine($"SoTien = {goiNap.SoTien}");

            long amount = Convert.ToInt64(goiNap.SoTien);
